...
function obtenerSoloLosMejores(estudiantes, nota1, nota2) {
    ...
    for (let i = 0; i < estudiantes.length; i++) {
        if (estudiantes[i].check1 >= nota1) {
            uno = estudiantes[i]["nombre"] + " " + estudiantes[i]["apellido"];
        }
    }
    for (let r = 0; r < estudiantes.length; r++) {
        if (estudiantes[r].check2 >= nota2) {
            dos = estudiantes[r]["nombre"] + " " + estudiantes[r]["apellido"];
        }
    }
    return [uno, dos];
